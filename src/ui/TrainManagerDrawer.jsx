              ...
              return (
                <section key={train.id} className="rounded-xl border border-white/10 bg-[#18263b] p-3">
                  ...
                  <div className="mt-3 grid grid-cols-5 gap-1">
                    <button type="button" onClick={() => cameraBus.emit({ type: 'focus', target: train.position, distance: 3.5 })} aria-label={`Focus Train ${index + 1}`} title="Focus camera" className="rounded-lg bg-[#34445a] p-2 hover:bg-[#465a73]"><img src={UI_ICONS.trainControls.focus} alt="" aria-hidden="true" className="ui-icon-image mx-auto h-4 w-4 object-contain" /></button>
                    <button type="button" onClick={() => onFollowTrain?.(followTrainId === train.id ? null : train.id)} aria-label={followTrainId === train.id ? `Unfollow Train ${index + 1}` : `Follow Train ${index + 1}`} title={followTrainId === train.id ? 'Unfollow train' : 'Follow train'} className={`rounded-lg p-2 ${followTrainId === train.id ? 'bg-purple-600' : 'bg-[#34445a] hover:bg-[#465a73]'}`}><img src={UI_ICONS.trainControls.follow} alt="" aria-hidden="true" className="ui-icon-image mx-auto h-4 w-4 object-contain" /></button>
                    <button type="button" onClick={() => trainManager.reverseTrain(train.id)} aria-label={`Reverse Train ${index + 1}`} title="Reverse direction" className="rounded-lg bg-[#34445a] p-2 hover:bg-[#465a73]"><img src={UI_ICONS.trainControls.reverse} alt="" aria-hidden="true" className="ui-icon-image mx-auto h-4 w-4 object-contain" /></button>
                    <button type="button" onClick={() => handleToggle(train.id)} aria-label={train.active ? `Stop Train ${index + 1}` : `Start Train ${index + 1}`} title={train.active ? 'Stop train' : 'Start train'} className={`rounded-lg p-2 ${train.active ? 'bg-[#846b2b]' : 'bg-[#276f4a]'} hover:brightness-110`}><img src={train.active ? UI_ICONS.trainControls.stop : UI_ICONS.trainControls.start} alt="" aria-hidden="true" className="ui-icon-image mx-auto h-4 w-4 object-contain" /></button>
                    <button type="button" onClick={() => handleDelete(train.id)} aria-label={`Delete Train ${index + 1}`} title="Delete train" className="rounded-lg bg-[#8f3b3b] p-2 hover:bg-[#ad4949]"><img src={UI_ICONS.trainControls.deleteCoach} alt="" aria-hidden="true" className="ui-icon-image mx-auto h-4 w-4 object-contain" /></button>
                  </div>
